...
PCH_REGISTER_COMPONENT(pch::Environment, PCH_ENVIROMENT_ID)
...
const char* Environment::get(const char* key, ErrorCode* errCode)
{
	...
	if (key == nullptr)
	{
		// 如果错误码输出指针不为空，则设置错误码为 PCH_NULLPTR
		if (errCode != nullptr){
			*errCode = PCH_NULLPTR;
		}
		return nullptr;
	...
	// 检查错误码输出指针是否为空，如果为空，则设置错误码为 PCH_SUCCESS
	if (errCode != nullptr){
		*errCode = PCH_SUCCESS;
	}
...
{
	// 检查环境变量名和值是否为空，如果为空，则返回 PCH_NULLPTR
