...
PCH_REGISTER_COMPONENT(pch::LoggerManager, PCH_LOGGERMANAGER_ID)

PCH_BEGIN_NAMESPACE


// 全局日志器：原子保护，避免 LoggerManager 析构与后台 WriteLog 调用之间的数据竞争/UAF
std::atomic<LoggerLevelCtrl*> _PCHLogger{ nullptr };
...
void WriteLog(LogLevel level, const char* fmt, ...)
{
	...
	// 开始解析变量列表
	va_start(vaList, fmt);
	// 一次性加载全局指针，避免在 writeLog 执行期间被另一线程清除
	LoggerLevelCtrl* logger = _PCHLogger.load(std::memory_order_acquire);
	if (logger) {
...
* @return 日志管理器
*/
LoggerManager::LoggerManager() : _defaultLogger("PCHCore")
{
	// 添加默认日志器输出后端
	_defaultLogger._loggerWrite.push_back(std::make_pair(&_consoleLogger, nullptr));
	// 设置默认日志器级别
	_defaultLogger._logLevel = LogLevel::Trace;
}

LoggerManager::~LoggerManager()
{
	// 安全网：如果 SystemShutdown 未被广播（例如直接调用 pch::api::Terminate），
	// 仍然需要在销毁此对象前清除全局 _PCHLogger，
	// 否则 WriteLog 将访问已释放的内存
	_PCHLogger.store(nullptr, std::memory_order_release);
}
...
*/
bool LoggerManager::setDefaultLoggerLevel(LogLevel level)
{
	// 加锁防止并发访问
	_mutex.lock();
	// 设置默认日志器级别
...
{
	// 如果日志器输出后端为空，返回 false
	if (!loggerWrite)	{
		return false;
	}
	// 加锁防止并发访问
	_mutex.lock();
	// 如果默认日志器已被获取，返回 false
...
* @return 成功返回 true，失败返回 false
*/
bool LoggerManager::setLoggerLevel(const char* logName, LogLevel level)
{
	// 如果日志器名称为空，返回 false
	if (!logName || strlen(logName) <= 0)
		return false;
...
* @return 始终返回 nullptr
*/
const pch::IMessage* LoggerManager::handleMessage(const pch::IMessage* msg)
{
	// 如果消息为空，返回 nullptr
	if (msg == nullptr) {
		return nullptr;
