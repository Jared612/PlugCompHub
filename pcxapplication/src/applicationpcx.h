/**
 * @file applicationpcx.h
 * @brief 基于 pcx::api 的应用骨架：配置、插件加载、系统消息与生命周期。
 */
#pragma once
...
constexpr int APP_SUCC               = 0;       //`start()` 成功或与 PCX_SUCCESS(0) 一致时的应用层返回值。
constexpr int APP_LOAD_CONFIG_FAILED = -10000;  //配置 `load()` 失败（如 TOML 解析错误）
constexpr int APP_API_INIT_FAILED    = -10000;  //`pcx::api::Initialize` 失败或对象管理器未就绪等初始化错误。
constexpr int APP_API_CHECK_INIT_ERR = -10001;  //主应用模式下对象管理器已存在，与「尚未 Initialize」的约定冲突。

/**
 * @brief 基于 pcx::api 的应用框架：配置、插件加载、系统消息与生命周期。
 * @details 是套在 pcx::api 外面的「应用启动流程」：用一份 ConfigBase/ConfigToml 配置，按约定把 PCX 跑起来并拉起业务。
 */
class ApplicationPCX
{
protected:
	/** @brief 消息处理函数类型 */
	typedef const pcx::IMessage* (ApplicationPCX::*FuncMessageHandle)(const pcx::IMessage* msg);
...
public:
	 ...
	 */
	explicit ApplicationPCX(const char* configPath);

	/**
	 * @brief 构造函数
	 * @param config 配置对象
	 */
	explicit ApplicationPCX(ConfigBase* config);

	/**
	 * @brief 析构函数
	 */
	virtual ~ApplicationPCX();
