 ...
 * @return 日志级别
 */
PcxLogLevel parseLogLevel(const std::string& val)
{
	if (val == "TRACE") return PcxLogLevel::Trace;
	if (val == "DEBUG") return PcxLogLevel::Debug;
	if (val == "INFO") return PcxLogLevel::Information;
	if (val == "WARN") return PcxLogLevel::Warning;
	if (val == "ERROR") return PcxLogLevel::Error;
	if (val == "FATAL") return PcxLogLevel::Fatal;
	if (val == "CLOSE" || val == "OFF") return PcxLogLevel::Off;
	return PcxLogLevel::Debug;
}
