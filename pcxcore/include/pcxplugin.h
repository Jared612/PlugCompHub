...
extern pcx::ComponentInfo* _pcx_componentInfoTable[]; \
extern "C" __PCX_EXPORT pcx::PluginInfo* pluginfo() { \
	static pcx::PluginInfo pinfo = { \
...
// Windows 下也通过 VARGS 实现 2/3 参数派发，消除与 Linux/macOS 的平台差异
#define PCX_PLUGIN_INFO_BEGIN(...) PCX_EXPAND(CONCAT(PCX_PLUGIN_INFO_BEGIN_,VARGS(__VA_ARGS__))(__VA_ARGS__))
#else
#define PCX_PLUGIN_INFO_BEGIN_3(NAME, VERSION,tableName) \
