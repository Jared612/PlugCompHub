/**
 * @file pcx.h
 * @brief 宿主侧**常规用法**：通过 `pcx::api` 动态加载 PCX 库并调用能力接口。
 * @details
 * 常规做法：只包含本文件，调用 `pcx::api::Initialize(库路径)` 等。运行时会**加载 PCX 动态库**，再按**函数名**
 *（如 `Initialize`）在库里查找地址并调用，一般**不必**在工程里预先链接 PCX 的导入库。
 *
 * 若你的工程是**把 PCX 和主程序编译链接在一起**，要在代码里**直接写** `Initialize`、`CreateObject` 等，
 * 则声明在 `pcxexport.h` 文件**最后一段**；那里返回值写成 `int`，与本文件里的 `ErrorCode`（本质上就是整数）一致。
 */
#pragma once
#include <stdarg.h>
#include "pcxexport.h"
...
// api单例类，作用是把 PCX 的全局能力封装成一组静态函数。
class api
{
public:
	/**
	* @brief 初始化PCX平台
	*
	* @param  pcxPath PCX库文件路径
	* @param  objectManager 会返回objectManager对象
	*
	* @return 错误码 PCX_SUCCESS 成功
	*			PCX_FAILED 初始化失败，详情见日志
	*/
	static inline ErrorCode Initialize(const char *pcxPath = PCX_LIB_NAME)
	{
		...
		instance._initializeFunc = (InitializeFunc)__linkto(pcxPath, "Initialize");
		// 同样找 Terminate 符号，保存到 _terminateFunc
		instance._terminateFunc = (TerminateFunc)__linkto(pcxPath, "Terminate");
	...
	*/
	static inline ErrorCode Terminate()
	{
		...
		ErrorCode err = instance._terminateFunc();   // 停止 PCX 运行时并释放内部资源
		...
		__unlinkfrom();                              // 释放 __linkto 缓存的动态库句柄，允许后续重新加载
		return err;                                  // 返回终止过程结果
	}

	/** 进程内单例；插件 `pluginit`（见 `pcxplugin.h`）需访问以注入 `IObjectManager*`。 */
	static api& get()
	{
		static api _api;
		return _api;
