 ...
 * @details
 * 组件管理器是 PCX 内核中「组件类型」的注册表：`componentID` → `Component`（对 `ComponentInfo` 的封装）。
...
public:
	 ...
	 * @return PCX_SUCCESS 命中延迟队列并成功回到注册表；
	 *         PCX_COMPONENT_NOTFOUND 延迟队列中没有对应项（调用方可再走常规 `registerComponent`）；
	 *         PCX_COMPONNET_ADYEXIST 同 ID 已在注册表中；其他错误码按参数校验返回。
	 */
