...
public:
	...
	{
		// 接口当前为 `uint16_t`，对超长目标列表做安全截断并记录警告，避免静默窄化为 0 等异常值；
		// 如业务确有 >UINT16_MAX 的接收者需求，应按 ABI 兼容方式升级接口到 `uint32_t`。
...
public:
	 ...
	 * @return `PCX_SUCCESS` 或错误码（如部分名解析失败仍可能入队）
	 */
	 ...
	 */
	virtual const pcx::IMessage* handleMessage(const pcx::IMessage* msg);
private:
	/**
	 ...
	 * @param[in] targetNames 接收方对象名列表（异步路径按名解析，回避 `ObjectInfo*` 被并发销毁的 UAF）。
	 * @param[in,out] msg 待入队消息；出参置空
	 * @note 匿名对象 `objName` 为空，被过滤；异步广播不覆盖匿名对象。
	 */
	void enqueueMessage(std::vector<std::string> targetNames, IMessage* &msg);

private:
	/** @brief 异步队列单元：一组目标名 + 同一条消息。 */
	struct MessagePack
	{
		std::vector<std::string> _targetNames; //接收方对象名列表（按名解析，避免持有裸 ObjectInfo*）
		IMessage* _msg;                        //待投递消息（处理完后释放）
	};
	...
	IMessageRelayer              *_messageRelayer = nullptr;  //可选的分布式消息中继。初始为 nullptr；收到 `SystemReady` 时按环境变量 `PCX_MESSAGE_RELAY`（缺省对象名同此字符串）`findObject` 绑定。未注册或查找失败则保持空，凡 `dstNodeID != 0` 及 join/leave 组等远程路径将失败。
};
