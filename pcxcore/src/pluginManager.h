...
public:
	...
	/**
	 * @brief 处理系统广播消息。
	 * @details 当前仅处理 `SystemShutdown`：逐个通知已加载插件执行 `pluginexit`，
	 *          清空插件侧 `pcx::api::_objectManager`，避免宿主销毁后插件悬挂引用。
	 */
	const pcx::IMessage* handleMessage(const pcx::IMessage* msg) override;

private:
	/**
	 * @brief 判断插件导出的任一组件是否仍在 `ObjectManager` 中存在活对象。
	 * @param[in] plugin 插件对象指针。
	 * @return 仍有活对象返回 true；否则 false。
	 */
