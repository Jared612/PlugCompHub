 ...
 * @details
 * 日志管理器为 PCX 内置日志能力的组织中心，按名维护 `LoggerLevelCtrl`，支持以 `.` 分段的名字继承父级配置，
...
void    WriteLog(PcxLogLevel level, const char* fmt, ...);  	// 日志入口函数，定义在 `loggerManager.cpp`，用于将日志写入到标准输出
...
public:
	// 将日志文本写到标准输出
	virtual void writeLog(PcxLogLevel level, const char* logText)
	{
		printf("%s", logText);
		printf("\n");
	}
...
public:
	...
	void fatal(const char* fmt, ...)
	{
		...
		va_start(vaList, fmt);	
		// 写入致命错误日志
		writeLog(PcxLogLevel::Fatal, fmt, vaList);
		// 结束解析变量列表
		va_end(vaList);
	}
	...
	void error(const char* fmt, ...)
	{
		...
		va_start(vaList, fmt);
		// 写入错误日志
		writeLog(PcxLogLevel::Error, fmt, vaList);
		// 结束解析变量列表
		va_end(vaList);
	}
	...
	void warn(const char* fmt, ...)
	{
		...
		va_start(vaList, fmt);
		// 写入警告日志
		writeLog(PcxLogLevel::Warning, fmt, vaList);
		// 结束解析变量列表
		va_end(vaList);
	}
	...
	void info(const char* fmt, ...)
	{
		...
		va_start(vaList, fmt);
		// 写入信息日志
		writeLog(PcxLogLevel::Information, fmt, vaList);
		// 结束解析变量列表
		va_end(vaList);
	}
	...
	{	
		...
		va_start(vaList, fmt);
		// 写入调试日志
		writeLog(PcxLogLevel::Debug, fmt, vaList);
		// 结束解析变量列表
		va_end(vaList);
	}
	...
	void trace(const char* fmt, ...)
	{
		...
		va_start(vaList, fmt);
		// 写入跟踪日志
		writeLog(PcxLogLevel::Trace, fmt, vaList);
		// 结束解析变量列表
		va_end(vaList);
	}

	// 写入日志
	virtual void writeLog(PcxLogLevel logLevel, const char* fmt, va_list vaList)
	{
		// 如果日志级别大于当前日志级别，则不写入
	...
	PcxLogLevel _logLevel = (PcxLogLevel)-1;  // 日志级别
...
/**
 * @class LoggerManager
 * @brief PCX 日志中枢：实现 `ILoggerManager`，并可通过消息调整日志行为（`IMessageHandler`）。
 * @details
 * 默认日志器名为 `pcxCore`，构造时挂接 `_consoleLogger`。对 `_loggerMap` 与默认项的更新由 `_mutex` 保护。
...
public:
	/**
	 * @brief 构造默认日志中枢。
	 * @details 默认日志器名为 `pcxCore`，挂接 `_consoleLogger`，级别为 `PcxLogLevel::Trace`。
	 */
	LoggerManager();
	...
	/**
	 * @brief 设置默认日志器（`pcxCore`）的最低输出级别。
	 * @param[in] level 级别
	 * @return 恒为 true
	 */
	virtual bool setDefaultLoggerLevel(PcxLogLevel level);
	 ...
	 */
	virtual bool setLoggerLevel(const char* logName,PcxLogLevel level);

	/**
	 * @brief 处理系统消息：`SystemReady` 时将全局 `WriteLog` 用的 `_pcxLogger` 指到 `pcxCore`；`SystemShutdown` 时清空并标记关闭。
	 * @param[in] msg 消息指针
	 * @return 恒为 nullptr
	 */
	const pcx::IMessage* handleMessage(const pcx::IMessage* msg);

	/**
	 * @brief 返回内置默认 `LoggerLevelCtrl`（名 `pcxCore`）的指针。
