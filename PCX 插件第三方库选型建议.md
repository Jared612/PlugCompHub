...
本文基于当前 PCX 仓库的实际架构，给出三类插件的第三方库选型建议：
...
如果目标是基于当前 PCX 架构，做一个实用、可维护、可扩展的插件体系，那么建议是：
