...
MCP_REGISTER_COMPONENT(mcp::Environment, MCP_ENVIROMENT_ID)
...
const char* Environment::get(const char* key, ErrorCode* errCode)
{
	...
	if (key == nullptr)
	{
		// 如果错误码输出指针不为空，则设置错误码为 MCP_NULLPTR
		if (errCode != nullptr){
			*errCode = MCP_NULLPTR;
		}
		return nullptr;
	...
	// 检查错误码输出指针是否为空，如果为空，则设置错误码为 MCP_SUCCESS
	if (errCode != nullptr){
		*errCode = MCP_SUCCESS;
	}
 ...
 * @return 成功返回 MCP_SUCCESS，失败返回 MCP_NULLPTR
 */
ErrorCode Environment::set(const char* key, const char* value)
{
	// 检查环境变量名和值是否为空，如果为空，则返回 MCP_NULLPTR
