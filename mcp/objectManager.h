 ...
 * @details
 * 对象管理器是 MCP 框架的核心子系统之一，负责管理所有对象的完整生命周期，
...
struct ObjectInfo
{
	...
	int         counter;    // 引用计数（当前主要用于查询，未做自动引用管理）。
...
public:
	 ...
	 * @param[in] obj 对象实例地址。
	 * @return 找到返回对象名称 C 字符串；未找到返回空字符串 ""。
	 */
...
private:
	std::recursive_mutex     _regMutex;                     //匿名对象列表的递归互斥锁
	std::vector<ObjectInfo*> _regObjs;                      //所有已注册对象（含具名和匿名）的列表，按注册顺序排列
	std::recursive_mutex     _regNamedMutex;                //保护具名对象映射表和地址映射表的递归互斥锁
	RegistedObjInfos         _regObjsMap;                   //对象名 -> ObjectInfo 的映射表，用于按名称快速查找
	std::unordered_map<void*, ObjectInfo*> _regObjAddrMap;  //对象地址 -> ObjectInfo 的映射表，用于按地址反查对象信息
};
