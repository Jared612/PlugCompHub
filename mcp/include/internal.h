...
/** IObjectArray / IObjectManager 在 interface.h（供 mcp.h 内联使用）。 */
...
public:
	...
	virtual bool setDefaultLoggerLevel(McpLogLevel level)                                                      = 0;  // 设置默认日志级别
	...
	virtual bool setLoggerLevel(const char* logName, McpLogLevel level)                                        = 0;  // 设置指定日志级别
