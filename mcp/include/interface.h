 ...
 * @details 在 `mcpcomponent.h` 之上展开协议与虚接口；扩展功能或对接 MCP 时常包含本头。
 */
...
// IMessageHandler 是外部组件接入 MCP 消息系统的“入口”。 外部最常见用法就是：你的组件继承它，然后实现 handleMessage 
...
public:
	...
	* @return - 回复的消息
	*/
	virtual const mcp::IMessage* handleMessage(const mcp::IMessage* msg) = 0;
};
...
public:
	...
	* @return MCP_SUCCESS - 成功
			  其他        - 错误码
	*/
	virtual ErrorCode sendMessage(uint32_t dstNodeID, const char* target, mcp::IMessage* req, mcp::IMessage** rsp, uint32_t timeout = 1000) = 0;
	...
	* @return MCP_SUCCESS - 成功
			  其他        - 错误码
	*/
	virtual ErrorCode postMessage(uint32_t dstNodeID, const char* target, mcp::IMessage* req) = 0;
	...
	* @return MCP_SUCCESS - 成功
			  其他        - 错误码
	*/
	virtual ErrorCode multicastRemoteMessageAsync(uint32_t dstNodeID, const char* targets[], int count, mcp::IMessage*& req) = 0;
	...
	* @return MCP_SUCCESS - 成功
			  其他        - 错误码
	*/
	virtual ErrorCode broadcastRemoteMessageAsync(uint32_t dstNodeID, mcp::IMessage*& req) = 0;

	/**
	* @brief 加入组
	* @param groupId - 组ID
	* @return 加入组的结果，MCP_SUCCESS为成功
	*/
...
public:
	...
	*/
	 ...
	 * @return 发送返回值，MCP_SUCCESS为成功，失败返回错误码
	*/
	...
	*/
	 ...
	 * @param msgPolicy - 消息发送规则
	 * @param timeout - 超时时间，单位：毫秒
	 * @return 组件对象查找结果，MCP_SUCCESS为成功
	*/
...
{
public:
	virtual void writeLog(mcp::McpLogLevel level, const char* logText) = 0;
...
{
public:
	virtual int formatLog(char* logBuffer, int bufferSize, const char* logName , mcp::McpLogLevel level, const char* fmt, va_list va) = 0;
};
...
/**
 * @brief 对象管理接口（供 mcp::api 内联封装使用，与内核实现一致）。
 */
