...
`pchcore` 是 PCH 框架的插件化运行时核心，主要负责：

- 核心组件启动与停止：`Initialize` / `Terminate` 对外导出，内部通过 `pchcoreStart` / `pchcoreStop` 装配和销毁核心对象。
...
`PluginManager::registerPlugin` 会先把插件写入 `_plugins`，然后逐个注册插件导出的组件。问题是组件注册失败时只记录日志，最终仍然返回 `PCH_SUCCESS`。
...
位置：`src/PCH.cpp`、`src/messagecenter.cpp`、`src/loggerManager.cpp`、`src/pluginManager.cpp`
...
- 插件侧 `pch::api::_objectManager` 清理路径不会触发，插件可能在宿主销毁后保留悬挂指针。
...
`pchcore` 的基础功能比较完整，已经覆盖插件化框架所需的核心模块：
